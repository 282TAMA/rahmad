...
for xd in range(10000):
	...
	l='Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/372.1.0.23.107;]'
...
def menu(id):
	...
	print('>> 5. Hasil Crack  ')
	print('>> 0. Keluar       ')
	_____mocha__tama_____ = input('\n>> Pilih : ')
	if _____mocha__tama_____ in ['1']:
		dump_massal()
	#elif _____mocha__tama_____ in ['2']:
		dump_follower()
	#elif _____mocha__tama_____ in ['3']:
		error()
	#elif _____mocha__tama_____ in ['4']:
		crack_file()
	elif _____mocha__tama_____ in ['5']:
		result()
	elif _____mocha__tama_____ in ['0']:
...
def dump_massal():
	...
	try:
		...
		print('>> Masukkan Angka Anjing, Malah Huruf, belajar dek ')
		exit()
	if jum<1 or jum>100:
...
def lah():
	print("\r"+balmond+m+" \x1b[1;95mTotal ID Yang Lu Kumpulin :\x1b[1;97m "+str(len(id))+"                     ")
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Sedang Menghancurkan Bumi {m}•{k}•{h}•{x} <<<<< ')
	print('')
